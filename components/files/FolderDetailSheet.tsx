...
export interface FolderDetailSheetProps {
   ...
   * them). Used by SharedWithMeView per backend spec §9.3: shared-with-me
   * items always have is_owner: false and the UI must offer a reduced
   * action set. Defaults to false, so FilesFeed's and StarredView's
...
export default function FolderDetailSheet({ folderId, onClose, onDeleted, onRenamed, readOnly = false }: FolderDetailSheetProps) {
                  ...
                  <li key={entry.id} className="rounded-lg border border-[var(--border-subtle)] px-3 py-2 text-sm">
                    <div className="flex items-center justify-between gap-3">
                      <span className="font-medium text-[var(--text-primary)]">
                        {tf.has(`audit.action.${entry.action}` as never) ? tf(`audit.action.${entry.action}` as never) : entry.action}
                      </span>
